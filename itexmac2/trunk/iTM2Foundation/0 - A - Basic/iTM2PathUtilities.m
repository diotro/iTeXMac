...
{iTM2_DIAGNOSTIC;
	//iTM2_START;
    return [isa iTM2_pathForSupportDirectory:subpath inDomain:domainMask withName:[self iTM2_bundleName] create:create];
}
@end
...
+ (void) preparePathUtilitiesCompleteInstallation;
{
	if([NSURL iTM2_swizzleInstanceMethodSelector:@selector(SWZ_iTM2ProjectDocumentKit_initWithCoder:)])
	{
		iTM2_MILESTONE((@"NSURL(iTM2PathUtilities)"),(@"Program inconsistancy"));
	}
}
@end
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  iTM2PATHServer
