...
{iTM2_DIAGNOSTIC;
	iTM2_INIT_POOL;
//iTM2_START;
	_iTM2MacroServerStorage = [[NSMutableDictionary alloc] init];
//iTM2_END;
	iTM2_RELEASE_POOL;
    return;
}
...
{iTM2_DIAGNOSTIC;
//iTM2_START;
	id element = [self macroActionForKey:key context:context ofCategory:category inDomain:domain];
	NSString * actionName = [[element attributeForName:@"ACT"] stringValue];
	if(![actionName length])
	{
		actionName = @"insertMacro:";
	}
	SEL action = NSSelectorFromString(actionName);
	id argument = [[[element elementsForName:@"ARG"] lastObject] stringValue];
	if(argument)
	{
		...
		{
			iTM2_LOG(@"No target for %@ with argument %@", actionName, argument);
		}
	}
	...
	{
		iTM2_LOG(@"No target for %@ with no argument", actionName);
	}
//iTM2_END;
    return NO;
}
...
//iTM2_START;
	...
	id storage = [self storageForContext:context ofCategory:category inDomain:domain];
	NSMenu * M = [storage objectForKey:@"./Menu"];
	if(!M)
	{
		NSString * directory = [iTM2MacroServerDirectoryName stringByAppendingPathComponent:domain];
		directory = [directory stringByAppendingPathComponent:category];
		directory = [directory stringByAppendingPathComponent:context];
		NSArray * RA = [[NSBundle mainBundle] allPathsForResource:@"Menu" ofType:@"xml" inDirectory:directory];
		if([RA count])
		{
			NSError * error = nil;
			NSXMLDocument * xmlDoc = [[[NSXMLDocument alloc] initWithContentsOfURL:[NSURL fileURLWithPath:[RA objectAtIndex:0]] options:0 error:&error] autorelease];
			if(error)
			{
				[SDC presentError:error];
			}
			M = [self macroMenuWithXMLElement:[xmlDoc rootElement] forContext:context ofCategory:category inDomain:domain error:&error];
		}
		if(!M)
		{
			M = [[[NSMenu allocWithZone:[NSMenu menuZone]] initWithTitle:@""] autorelease];
			NSDictionary * D = [SUD dictionaryForKey:[NSString pathWithComponents:[NSArray arrayWithObjects:@"Recent", domain, nil]]];
			NSEnumerator * E = [D keyEnumerator];
			NSString * K;
			while(K = [E nextObject])
			{
				NSArray * RA = [D objectForKey:K];
				NSMenuItem * MI = [[[NSMenuItem allocWithZone:[M zone]] initWithTitle:K action:@selector(___insertMacro:) keyEquivalent:@""] autorelease];
		...
		if(!M)
			M = [[[NSMenu allocWithZone:[NSMenu menuZone]] initWithTitle:@""] autorelease];
		[storage setObject:M forKey:@"./Menu"];
		M = [storage objectForKey:@"./Menu"];
		[M insertItemWithTitle:@"" action:NULL keyEquivalent:@"" atIndex:0];
	}
//iTM2_END;
    return M;
}
...
//iTM2_START;
	...
	if([RA isKindOfClass:[NSArray class]] && [RA count])
	{
		...
		if([key length])
		{
			[iTM2MacrosServer executeMacroWithKey:key forContext:context ofCategory:category inDomain:domain];
...
//iTM2_START;
	...
	if(R1.length)
	{
		...
		if(R2.length && (R1.location += R1.length, (R2.location > R1.location)))
		{
			...
			NSString * category = [name substringWithRange:R1];
			NSString * domain = [self domain];
			return [iTM2MacrosServer macroMenuForContext:@"" ofCategory:category inDomain:domain error:nil];
		}
	}
//iTM2_END;
