...
//iTM2_START;
    ...
    if([variants isKindOfClass:[NSArray class]])
    {
        ...
        while(variant = [e nextObject])
        {
			...
			{
				iTM2_REPORTERROR(1, ([NSString stringWithFormat:@"Unknown inspector type:\n%@\nfor mode:\n%@\nfor variants:\n%@", type, mode,variants]), nil);
			}
        }
    }
...
{iTM2_DIAGNOSTIC;
//iTM2_START;
    if([SUD boolForKey:iTM2UDSmartUndoKey])
    {
        NSWindow * W = [NSApp mainWindow];
        if(![[[W windowController] document] isDocumentEdited])
        {
