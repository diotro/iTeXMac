...
//iTM2_START;
	if(!iTM2StringEncodingMissingFormat)
	{
		...
		if(OK)
		{
			iTM2_MILESTONE((@"iTM2StringFormatControllerLocalizedMenuItems"),(@"No Localized menu items available for iTM2StringFormatController"));
		}
...
terminate:
					...
					result = NO;
					canStringEncoding = YES;
					iTM2_LOG(@"The hard coded string encoding %@ could not be honoured:%@, using %@ instead",
						hardStringEncodingString,
...
//iTM2_START;
    if([tv isEqual:actualTableView])
    {
        if([tv isEqual:[info draggingSource]])
        {
			...
			[IS addIndexesInRange:NSMakeRange(row,[MRA count])];
        ...
        else
        {
            ...
            [tv reloadData];
			NSMutableIndexSet * IS = [NSMutableIndexSet indexSet];
			[IS addIndexesInRange:NSMakeRange(row,length)];
			[tv selectRowIndexes:IS byExtendingSelection:NO];
            [self updateChangeCount:NSChangeDone];
            [self iTM2_validateWindowsContents];
            return YES;
...
+ (void)prepareStringFormatKitCompleteInstallation;
{
	[iTM2StringFormatController completeInitialization];
}
@end
