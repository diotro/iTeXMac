...
+ (void)prepareAutoKitCompleteInstallation;
{
	if([NSDocument iTM2_swizzleInstanceMethodSelector:@selector(SWZ_iTM2Auto_writeToURL:ofType:forSaveOperation:originalContentsURL:error:)]
		&& [NSDocument iTM2_swizzleInstanceMethodSelector:@selector(SWZ_iTM2Auto_readFromURL:ofType:error:)])
	{
		iTM2_MILESTONE((@"NSDocument(iTM2AutoKit)"),(@"No readFromURL:... patch, potential problems"));
	}	
}
@end
