...
//iTM2_START;
	...
	if(TV)
	{
		NSUInteger line = [sender intValue];
		line = MAX(2,line)-1;
...
{
	if([NSFormatter iTM2_swizzleInstanceMethodSelector:@selector(SWZ_iTM2TextField_attributedStringForObjectValue:withDefaultAttributes:)])
	{
		iTM2_MILESTONE((@"NSFormatter(iTM2TextField)"),(@"Attributes for nil object value are not handled?"));
	}
    return;
}
