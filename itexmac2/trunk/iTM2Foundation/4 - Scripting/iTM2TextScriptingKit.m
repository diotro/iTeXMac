...
"*/
{
    ...
    if([target isKindOfClass: [NSTextStorage class]] && [S length])
    {
        ...
        {
            if(findRangeValue)
                [target setSelectedRange: NSMakeRange(NSMaxRange([findRangeValue rangeValue]), 0)];
            [TF findPrevious: nil];
        }
        else
        {
            if(findRangeValue)
                [target setSelectedRange: NSMakeRange([findRangeValue rangeValue].location, 0)];
            [TF findNext: nil];
        }
    ...
    else
    {
        ...
        else if([O isKindOfClass: [NSPositionalSpecifier class]])
        {
            if([[O insertionKey] isEqualToString: @"characters"])
            {
                ...
                {
                    [findRangeValue autorelease];
                    findRangeValue = [[NSValue valueWithRange: NSMakeRange([O insertionIndex], 0)] retain];
                    return findTarget = [container retain];
                }
                else
                {
    ...
    else
    {
        ...
        if([O isKindOfClass: [NSPositionalSpecifier class]])
        {
            if([[O insertionKey] isEqualToString: @"characters"])
            {
                ...
                {
                    return findString =
                        [[[container string] substringWithRange: NSMakeRange([O insertionIndex], 1)] retain];
                }
                else
                {
                    [self setScriptErrorNumber: 4];
            ...
            if([[O insertionKey] isEqualToString: @"characters"])
            {
                ...
                if([container isKindOfClass: [NSTextStorage class]])
                {
                    ...
                    else
                    {
                        ...
                        insertionRangeValue = [[NSValue valueWithRange: NSMakeRange([O insertionIndex], 0)] retain];
...
//NSLog(@"%@ %#x IS DONE 4", __iTM2_PRETTY_FUNCTION__, self);
    if([MD count])
        [[self insertionTarget] insertMacro: MD inRange: [[self insertionRangeValue] rangeValue]];
	...
	iTM2TextFinder * TF = [[[iTM2TextFinder alloc] initWithTextView: [target mainTextView]] autorelease];
        ...
        if([self changeAllFlag])
        {
            if(findRangeValue)
                [target setSelectedRange: NSMakeRange(NSMaxRange([findRangeValue rangeValue]), 0)];
            [TF replaceAll: nil];
            return [NSNumber numberWithInt: [TF numberOfOps]];
        }
        ...
        if([TF lastFindWasSuccessful])
            [TF replace: nil];
        return [NSNumber numberWithInt: ([TF lastFindWasSuccessful]? 1: 0)];
    }
    else
...
"*/
{
    ...
    else if([O isKindOfClass: [NSIndexSpecifier class]])
    {
        id container = [[O containerSpecifier] objectsByEvaluatingSpecifier];
        if([container respondsToSelector: @selector(setSelectedRangeValue:)])
            [container setSelectedRangeValue: [NSValue valueWithRange: NSMakeRange([O evaluatedPosition], 1)]];
        else
        {
            [self setScriptErrorNumber: 2];
            [self setScriptErrorString: [NSString stringWithFormat: @"Don't know how to select %@", O]];
