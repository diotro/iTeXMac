...
//iTM2_START;
	...
	if([wrapperName length])
	{
		...
		// we should not have reached this part of code unless there is a new design
...
//iTM2_START;
	...
	if([wrapperName length])
	{
		wrapperName = [wrapperName stringByDeletingPathExtension];
		wrapperName = [wrapperName stringByDeletingLastPathComponent];
		[self setOldProjectName:wrapperName];
		[self setDocumentIsMaster:YES];
		creationMode = iTM2ToggleNewProjectMode;
	}
	else if([availableProjects count]==0)
	{
		[self setOldProjectName:@""];
	...
	{
		NSString * preferredProject = [SUD stringForKey:@"iTM2NewProjectLastChoice"];
		if(![[availableProjects allValues] containsObject:preferredProject])
		{
			NSArray * Ks = [availableProjects allKeys];
			Ks = [Ks sortedArrayUsingSelector:@selector(compare:)];
			NSString * K = [Ks objectAtIndex:0];
			preferredProject = [availableProjects objectForKey:K];
		}
		[self setOldProjectName:preferredProject];
	}
...
{iTM2_DIAGNOSTIC;
//iTM2_START;
	[sender setState: ([sender tag] == [self creationMode]? NSOnState: NSOffState)];
	BOOL result = NO;
	switch([sender tag])
	{
		case 0:
		{
			id availableProjects = [self availableProjects];
			result = ([availableProjects count]>0);
			break;
		}
		case 1:
			result = ![self belongsToAWrapper];// new project
			break;
		default:
			result = YES;// standalone document
			break;
	}
	return result;
}
