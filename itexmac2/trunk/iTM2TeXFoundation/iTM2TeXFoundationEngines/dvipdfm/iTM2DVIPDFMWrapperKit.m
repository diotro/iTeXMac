...
//iTM2_START;
	...
	[MD addEntriesFromDictionary:
			[NSDictionary dictionaryWithObjectsAndKeys:
				[NSNumber numberWithInt:0], iTM2Dvipdfm_open_bookmark_depth,
				[NSNumber numberWithInt:0], iTM2Dvipdfm_option_flags,
				[NSNumber numberWithInt:0x003C], iTM2Dvipdfm_permission_flags,
				[NSNumber numberWithBool:NO], iTM2Dvipdfm_enable_encryption,
				[NSNumber numberWithBool:NO], iTM2Dvipdfm_include_thumbnails,
				[NSNumber numberWithInt:3], iTM2Dvipdfm_minor_version,
...
- (IBAction)toggleOptionFlagFromTag:(id)sender;
/*"Description forthcoming.
Version History: jlaurens AT users DOT sourceforge DOT net
- 1.4: Thu Nov 18 07:53:25 GMT 2004
To Do List:
...
{iTM2_DIAGNOSTIC;
//iTM2_START;
	int options = [self optionFlags];
	int mask = [sender tag];
	if(options & mask)
		options &= ~mask;
	else
		options |= mask;
	[self setOptionFlags:options];
//iTM2_END;
	return;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  validateToggleOptionFlagFromTag:
- (BOOL)validateToggleOptionFlagFromTag:(id)sender;
/*"Description forthcoming.
Version History: jlaurens AT users DOT sourceforge DOT net
- 1.4: Thu Nov 18 07:53:25 GMT 2004
To Do List:
...
//iTM2_START;
	int options = [self optionFlags];
	int mask = [sender tag];
	[sender setState:(options & mask?NSOnState:NSOffState)];
//iTM2_END;
	return YES;
}
@end
