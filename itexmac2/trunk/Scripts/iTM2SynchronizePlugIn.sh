...
then
    echo "warning: iTeXMac2 ERROR, Usage is \"iTM2SynchronizeFramework.sh iTM2_name\""
    exit -1
fi
FULL_PRODUCT_NAME="${BASE_PRODUCT_NAME}.iTM2"
pushd "`dirname "$0"`"/..
SRC_TREE_ROOT="`pwd`"
...
echo "warning: iTeXMac2 INFO, expected executable:"
echo "warning: ${FULL_PRODUCT_DIR}/${FULL_PRODUCT_NAME}/Contents/MacOS/${BASE_PRODUCT_NAME}"
if [ -d "${FULL_PRODUCT_DIR}/${FULL_PRODUCT_NAME}" ]
then
	if [ -x "${FULL_PRODUCT_DIR}/${FULL_PRODUCT_NAME}/Contents/MacOS/${BASE_PRODUCT_NAME}" ]
	then
...
then
	...
	exit 1
elif [ -x "${FULL_PRODUCT_DIR}/${FULL_PRODUCT_NAME}/Contents/MacOS/${BASE_PRODUCT_NAME}" ]
then
	echo "warning: iTeXMac2 INFO, building ${FULL_PRODUCT_NAME} if needed... SUCCEEDED"
	exit 0
else
	echo "warning: iTeXMac2 INFO, building ${FULL_PRODUCT_NAME} if needed... FAILED with missing executable"
	echo "warning: ${FULL_PRODUCT_DIR}/${FULL_PRODUCT_NAME}/Contents/MacOS/${BASE_PRODUCT_NAME}"
	exit 2
fi
