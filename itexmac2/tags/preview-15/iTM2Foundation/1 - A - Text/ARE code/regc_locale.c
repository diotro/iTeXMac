 ...
 *
 * RCS: @(#) $Id: regc_locale.c,v 1.1 2003/02/03 12:18:42 jlaurens Exp $
 */
...
int cases;			/* case-independent? */
{
	...
	for (c = a; c <= b; c++) {
		addchr(cv, c);
		lc = iTM2UniCharToLower((chr)c);
		uc = iTM2UniCharToUpper((chr)c);
		tc = iTM2UniCharToTitle((chr)c);
		if (c != lc) {
			addchr(cv, lc);
		}
		if (c != uc) {
...
size_t len;			/* exact length of comparison */
{
	...
	for (xp = x, yp = y, i = len; i > 0; i--)
		if (iTM2UniCharToLower(*xp++) != iTM2UniCharToLower(*yp++))
			return 1;
	return 0;
}
