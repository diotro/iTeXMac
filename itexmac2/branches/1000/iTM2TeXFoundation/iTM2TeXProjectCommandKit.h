...
@interface iTM2TeXPCommandsInspector: iTM2Inspector <NSWindowDelegate>
@end
...
/*! 
    ...
    @discussion This object is responsible of one of the commands iTeXMac2 knows of.
				...
				- .iTM2/bin(to be changed into $dotFolder4iTM3/bin)
				...
				The launcher scans all the environment variable names for names matching the extended regexp ShellScript_4iTM3(.*)
				then it copies the contents in .iTM2/bin, in shell scripts named iTM2_\1
				(for example echo "${ShellScript_Command_04iTM3}" > ".iTM2/bin/Command_04iTM3")
				Then the manager sends the appropriate command to the launcher, for example the command
				[fermat:~] jlaurens% Launch4iTM3 Compile
				will let Launch4iTM3 perform a $Compile4iTM3 whatever the contents of this environemnt variable can be.
				...
				the Compile4iTM3 environment variable will be defined accordingly.
				...
				The customized shell scripts end with names Command_4iTM3? (or BaseCommand_4iTM3? if they come from the base project)
