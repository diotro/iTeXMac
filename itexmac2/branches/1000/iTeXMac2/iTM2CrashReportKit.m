...
//START4iTM3;
	...
	id attributes = [DFM attributesOfItemAtPath:crashPath error:NULL];
	...
	if (modificationDate) {
		if (!lastCrashDate || ([lastCrashDate compare:modificationDate] == NSOrderedAscending)) {
			...
			for (NSString * component in availableLogs) {
				NSString * fullPath = [consoleDir stringByAppendingPathComponent:component];
				attributes = [DFM attributesOfItemAtPath:fullPath error:NULL];
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  send:
- (IBAction)send:(NSControl *)sender;
/*"Description Forthcoming.
Version history: jlaurens AT users DOT sourceforge DOT net
Latest Revision: Sun Mar 14 17:17:19 UTC 2010
To Do List:
"*/
{DIAGNOSTIC4iTM3;
//START4iTM3;
