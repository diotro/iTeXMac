...
//LOG4iTM3(@"variant is: %@", variant);
	...
	{
		stylePath = [[stylePath stringByAppendingPathComponent: variantComponent] stringByResolvingSymlinksAndFinderAliasesInPath4iTM3];
		BOOL isDir = NO;
...
//LOG4iTM3(@"stylePath: %@", stylePath);
    for(NSString * p in [DFM contentsOfDirectoryAtPath: stylePath error:NULL])
        if([[p pathExtension] isEqualToString: iTM2TextAttributesSymbolsExtension])
        {
            ...
            {
                LOG4iTM3(@"We have loaded symbols attributes at path: %@", symbolAttributesPath);
...
//START4iTM3;
    ...
    if(location<S.length)
    {
		if(previousMode == kiTM2TeXBeginCommandInput)
		{
			...
			while(end<S.length && [[NSCharacterSet TeXLetterCharacterSet4iTM3] characterIsMember: [S characterAtIndex: end]])
				++end;
			if(end == start+5)
			{
				if([@"input" isEqual: [S substringWithRange: iTM3MakeRange(start, end - start)]])
				{
					if(lengthRef)
						* lengthRef = end - start;
...
//NSLog(@"2: nextMode: %u, previousMode: %u", nextMode, previousMode);
				...
				else if(previousMode == kiTM2TeXBeginCommandInput)
				{
					if(nextModeRef)
					{
						...
						while(end<S.length && [[NSCharacterSet TeXLetterCharacterSet4iTM3] characterIsMember: [S characterAtIndex: end]])
							++end;
						if((end == start+5) && ([@"input" isEqual: [S substringWithRange: iTM3MakeRange(start, end - start)]]))
...
//START4iTM3;
    ...
    switch(switcher)
    {
        ...
        case kiTM2TeXBeginCommentInput:
        {
            ...
            if(aRangePtr)
                * aRangePtr = iTM3MakeRange(aLocation, endOffset - aLocation);
        ...
        default:
            LOG4iTM3(@"Someone is asking for mode: %u (%u)", mode, switcher);
...
//START4iTM3;
    ...
    switch(switcher)
    {
        ...
        case kiTM2TeXBeginCommandInput:
        {
            ...
            {
                NSRange r1;
                NSInteger nextMode = [self syntaxModeAtIndex: aLocation longestRange: &r1] & ~kiTM2TeXErrorsMask;
        ...
        case kiTM2TeXCommandInput:
        {
            if(r.location)
            {
				...
				if(aLocation+2<_TextStorage.length)
				{
					...
					{
						NSRange r1 = iTM3MakeRange(r.location, r.length + 3);
						NSDictionary * D = [_AS attributesForSymbol: [[_TextStorage string] substringWithRange: r1]];
						if(D)
						{
							if(aRangePtr)
        ...
        case kiTM2TeXShortCommandInput:
        {
            ...
            {
                NSRange r = iTM3MakeRange(aLocation-1, 2);
				++aLocation;
				if(aLocation+2<_TextStorage.length)
				{
					if(([[_TextStorage string] characterAtIndex: aLocation] == '{')
        ...
        default:
            LOG4iTM3(@"Someone is asking for mode: %u", switcher);
            return [_AS attributesForMode: [_iTM2TeXModeForModeArray objectAtIndex: kiTM2TeXErrorInput]];
    }
}
@end
