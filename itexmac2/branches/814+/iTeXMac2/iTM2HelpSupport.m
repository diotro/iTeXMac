...
@end
static iTM2MailController * sharedMailController4iTM3;
@implementation iTM2MailController
+ (id)sharedMailController;
{
	return sharedMailController4iTM3?:(sharedMailController4iTM3 = [self.alloc initWithWindowNibName:NSStringFromClass(self)]);
}
- (id)initWithWindow:(NSWindow *)window;
{
	if(sharedMailController4iTM3)
	{
		return sharedMailController4iTM3;
	}
	else if(self = [super initWithWindow:window])
	{
		...
		NSInteger emailIndex = [mailProperty indexForIdentifier: primaryIdentifier]; 
		NSString * mailSender = [mailProperty valueAtIndex: emailIndex];
		[self setMailSender:mailSender];
		[self setCanSend:YES];
	}
	return sharedMailController4iTM3 = self;
}
- (void)mailForHelp;
{
