...
/*!
    @method     regExWithSearchPattern:option:error:
    @abstract   The cached regular expression wrapper for the given pattern.
    @discussion Discussion forthcoming. The cached version is not thread safe.
    @param      pattern is a regular expression pattern, according to <a href="http://icu.sourceforge.net/userguide/regexp.html">ICU rules</a>
    @param      flags is the list of options (see above)
    @param      errorRef is a pointer to an error object set on return incase of error
	@result     nil is returned when an error occurred
*/
+ (id)regExWithSearchPattern:(NSString *)pattern option:(NSUInteger)flags error:(NSError **)errorRef;

/*!
    @method     regExWithSearchPattern:error:
    @abstract   The cached regular expression wrapper for the given pattern.
    @discussion Discussion forthcoming. The cached version is not thread safe.
    @param      pattern is a regular expression pattern, according to <a href="http://icu.sourceforge.net/userguide/regexp.html">ICU rules</a>
    @param      errorRef is a pointer to an error object set on return incase of error
...
/*!
    @method     regExForKey:error:
    @abstract   The cached regular expression wrapper for the given pattern key.
    @discussion The real pattern is found in a ressource file. The cached version is not thread safe.
                The resource file is found in the bundle of the receiver.
    @param      patternKey is a Key
    @param      errorRef is a pointer to an error object set on return incase of error
	@result     nil is returned when an error occurred
*/
...
/*!
    @method     regExForKey:inBundle:error:
    @abstract   The cached regular expression wrapper for the given pattern key.
    @discussion The real pattern is found in a ressource file. The cached version is not thread safe.
                When no regex is found, nil is returned and errorRef points to an autoreleased error.
                If a regex is found, what errorRef points to is undefined.
    @param      patternKey is a Key
...
/*!
    @method     setInputString: range:(NSRange)range
...
/*!
    @property   groupNames
    @abstract   Internally used by regexForKey:... methods. Abstract forthcoming.
    @discussion Discussion forthcoming.
    @result     The names of the groups
*/
@property (assign,readonly) NSArray * groupNames;

/*!
    @property   commentGroupIndexes
    @abstract   Internally used by regexForKey:... methods. Abstract forthcoming.
    @discussion Discussion forthcoming.
    @result     The names of the groups
*/
@property (assign,readonly) NSIndexSet * commentGroupIndexes;

/*!
    @method     matchString:
    @abstract   One shot matcher
    @discussion This is a convenient method, for one shot match. It sets the input string, test.
				When you have terminated to work with the receiver, send the receiver a forget message.
...
/*!
    @method     forget
    @abstract   Let the reciever forget its input string and replacement pattern
    @discussion When your work with a one shot RE is complete, send this message in order to clean its internals.
                This is particularly important for cached RE's. 
    @param      None.
    @result     None
*/
- (void)forget;
...
/*!
    ...
    @abstract   Returns YES if the input string matches the receiver's pattern at the given index
    ...
    @param      index is the location where the search will start.
                If a match is found, its location is greater than or equal to index.
                This is relative to the begining of the search string range as eventually given by -setInputString:range:.
    @param      yorn, pass YES if you want the match to extend to the end of the input string.
    @result     yorn
*/
- (BOOL)matchesAtIndex:(NSInteger)index extendToTheEnd:(BOOL)yorn;
...
/*!
    ...
    @result     yorn
*/
- (BOOL)nextMatchAfterIndex:(NSInteger)index;
...
extern NSString * const iTM2RegExpEOLKey;
extern NSString * const iTM2RegExpResourceName;
...
/*!
    @method     replaceOccurrencesOfICURegEx:error:
...
*/
- (NSUInteger)replaceOccurrencesOfICURegEx:(ICURegEx *)aRegEx error:(NSError **)errorRef;
...
/*!
	...
	@result     the number of replacements
*/
- (NSUInteger)replaceOccurrencesOfICUREPattern:(NSString *)target withPattern:(NSString *)replacement options:(NSUInteger)opts range:(NSRange)searchRange error:(NSError **)errorRef;

@end

@interface NSObject(ICURegEx)

/*!
    @method     ICURegExForKey4iTM3:error:
    @abstract   The cached regular expression wrapper for the given pattern key.
    @discussion Convenient method. Shortcut to +[ICURegEx regExForKey:inBundle:error:]
                where the bundle argument is the bundle of the receiver.
    @param      patternKey is a Key
    @param      errorRef is a pointer to an error object set on return in case of an error
	@result     nil is returned when an error occurred
*/
