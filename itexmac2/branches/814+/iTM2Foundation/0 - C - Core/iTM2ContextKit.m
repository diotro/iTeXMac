...
//START4iTM3;
//END4iTM3;
    return [self getContext4iTM3ValueForKey:aKey domain:mask error:RORef];
}
...
//START4iTM3;
	...
	if (mask & iTM2ContextStandardLocalMask) {
		...
		if ((self != context4iTM3Manager) && (SUD != context4iTM3Manager) && (result = [context4iTM3Manager context4iTM3ValueForKey:aKey domain:mask error:RORef])) {
			return result;
		}
	}
	if (mask & iTM2ContextDefaultsMask) {
		result = [SUD context4iTM3ValueForKey:aKey domain:mask error:RORef];
	}
    return result;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  takeContext4iTM3Value:forKey:domain:error:
...
//LOG4iTM3(@"self.context4iTM3Dictionary is:%@", self.context4iTM3Dictionary);
	NSUInteger didChange = [self setContext4iTM3Value:object forKey:aKey domain:mask error:RORef];
	if (didChange) {
		id afterObject = [self context4iTM3ValueForKey:aKey domain:mask error:RORef];
...
//START4iTM3;
	...
	if (mask & iTM2ContextStandardLocalMask) {
		...
		if (context4iTM3Manager && self != context4iTM3Manager && SUD != context4iTM3Manager) {
			...
			didChange |= [context4iTM3Manager takeContext4iTM3Value:object forKey:aKey domain:mask error:RORef];
		}
	}
	if (mask & iTM2ContextDefaultsMask) {
		didChange &= ~iTM2ContextNoContextMask;
		didChange |= [SUD takeContext4iTM3Value:object forKey:aKey domain:mask error:RORef];
	}
//LOG4iTM3(@"self.context4iTM3Dictionary is:%@", self.context4iTM3Dictionary);
...
{DIAGNOSTIC4iTM3;
//START4iTM3;
	NSDictionary * D = [self context4iTM3ValueForKey:aKey domain:mask ROR4iTM3];
//END4iTM3;
...
//START4iTM3;
    ...
    [self takeContext4iTM3Value:D forKey:aKey domain:mask ROR4iTM3];
//END4iTM3;
    return;
}
...
{DIAGNOSTIC4iTM3;
	...
	if (mask & iTM2ContextStandardLocalMask) {
		...
		id context4iTM3Manager = self.context4iTM3Manager;
		if ((context4iTM3Manager != self) && (context4iTM3Manager != SUD)
		   && ((value = [context4iTM3Manager context4iTM3ValueForKey:aKey domain:mask ROR4iTM3]))
...
{DIAGNOSTIC4iTM3;
	//START4iTM3;
    [self takeContext4iTM3Value:[NSNumber numberWithInteger:value] forKey:aKey domain:mask ROR4iTM3];
    return;
}
...
{DIAGNOSTIC4iTM3;
	//START4iTM3;
    [self takeContext4iTM3Value:[NSNumber numberWithUnsignedInteger:value] forKey:aKey domain:mask ROR4iTM3];
    return;
}
...
{DIAGNOSTIC4iTM3;
//START4iTM3;
    [self takeContext4iTM3Value:[NSNumber numberWithFloat:value] forKey:aKey domain:mask ROR4iTM3];
    return;
}
...
{DIAGNOSTIC4iTM3;
//START4iTM3;
    [self takeContext4iTM3Value:[NSNumber numberWithBool:value] forKey:aKey domain:mask ROR4iTM3];
    return;
}
...
//START4iTM3;
    NSInvocation * I;
	[[NSInvocation getInvocation4iTM3:&I withTarget:self retainArguments:NO] loadContext4iTM3Error:RORef];
    BOOL result = YES;
...
//START4iTM3;
	...
	if ([self context4iTM3ValueForKey:aKey domain:iTM2ContextPrivateMask ROR4iTM3])
	{
		return NSOffState;
	}
...
//START4iTM3;
	if ([self context4iTM3BoolForKey:aKey domain:iTM2ContextPrivateMask]) {
		...
		[self takeContext4iTM3Value:nil forKey:aKey domain:iTM2ContextPrivateMask ROR4iTM3];
		return;
	}
	if ([self context4iTM3BoolForKey:aKey domain:iTM2ContextAllDomainsMask&~iTM2ContextPrivateMask]) {
		[self takeContext4iTM3Value:nil forKey:aKey domain:iTM2ContextPrivateMask ROR4iTM3];
...
"*/
{DIAGNOSTIC4iTM3;
    return [self.window saveContext4iTM3Error:RORef];
}
@end
...
@implementation NSApplication(iTM2ContextKit)
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  currentContext4iTM3Manager
- (id)currentContext4iTM3Manager;
/*"Returns the context4iTM3Manager of its window.
Version history: jlaurens AT users DOT sourceforge DOT net
Révisé par itexmac2: 2010-12-27 23:45:54 +0100
To Do List:
"*/
{DIAGNOSTIC4iTM3;
//START4iTM3
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  context4iTM3ValueForKey:domain:error:
- (id)context4iTM3ValueForKey:(NSString *)aKey domain:(NSUInteger)mask error:(NSError **)RORef;
/*"Description forthcoming.
Version history: jlaurens AT users DOT sourceforge DOT net
- 1.1.a6: 03/26/2002
...
//START4iTM3;
    NSError * ROR = nil;
	if ([NSDocument swizzleInstanceMethodSelector4iTM3:@selector(SWZ_iTM2ContextKit_canCloseDocumentWithDelegate:shouldCloseSelector:contextInfo:) error:self.RORef4iTM3])
	{
...
//START4iTM3;
	...
	id result = nil;
	if ((result = [super getContext4iTM3ValueForKey:aKey domain:mask&~iTM2ContextStandardDefaultsMask error:RORef])) {
		return result;
	}
	...
	if (mask & iTM2ContextExtendedDefaultsMask) {
		...
		NSString * type4URL = [SDC typeForContentsOfURL:self.fileURL error:RORef];
	...
	result = [super getContext4iTM3ValueForKey:aKey domain:mask error:RORef];// debug design
    return result;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  setContext4iTM3Value:forKey:domain:error:
- (NSUInteger)setContext4iTM3Value:(id)object forKey:(NSString *)aKey domain:(NSUInteger)mask error:(NSError **)RORef;
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  documentCompleteSaveContext4iTM3Error:
...
//START4iTM3;
    BOOL result = YES;
    NSInvocation * I;
    [[NSInvocation getInvocation4iTM3:&I withTarget:self] documentCompleteSaveContext4iTM3Error:RORef];
    I.selector = @selector(saveContext4iTM3Error:);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=  documentCompleteLoadContext4iTM3Error:
- (BOOL)documentCompleteLoadContext4iTM3Error:(NSError **)RORef;
/*"Description forthcoming.
Version History: jlaurens AT users DOT sourceforge DOT net
Révisé par itexmac2: 2010-12-02 00:54:58 +0100
To Do List:
"*/
{DIAGNOSTIC4iTM3;
//START4iTM3;
    BOOL result = YES;
    NSInvocation * I;
    [[NSInvocation getInvocation4iTM3:&I withTarget:self] documentCompleteLoadContext4iTM3Error:RORef];
    I.selector = @selector(loadContext4iTM3Error:);
    for (NSWindowController * WC in self.windowControllers) {
        [I invokeWithTarget:WC];
        if (result) {
            [I getReturnValue:&result];
