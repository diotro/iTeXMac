/*
//
//  @version Subversion: $Id: iTM2HelpKit.m 424 2007-03-12 23:03:11Z jlaurens $ 
//
...
//START4iTM3;
    ...
    buttonHelp.action = @selector(showHelp:);
...
//START4iTM3;
    ...
    result.view = self.buttonHelp;
    result.minSize = result.maxSize = [[result view] frame].size;
    result.toolTip = NSLocalizedStringFromTableInBundle(iTM2ToolbarShowHelpItemIdentifier, @"Basic",
        self.classBundle4iTM3, nil);
    result.paletteLabel = NSLocalizedStringFromTableInBundle(iTM2ToolbarShowHelpItemIdentifier, @"Basic",
        self.classBundle4iTM3, nil);
    result.label = NSLocalizedStringFromTableInBundle(iTM2ToolbarShowHelpItemIdentifier, @"Basic",
        self.classBundle4iTM3, nil);
    return result;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= showHelp:
...
//START4iTM3;
    OSStatus status = AHGotoPage(
        (CFStringRef)[iTeXMac2HelpBookName stringWithSubstring4iTM3:@" " replacedByString:@"%20"],
        (CFStringRef)[object stringWithSubstring4iTM3:@" " replacedByString:@"%20"],
        (CFStringRef)[anchor stringWithSubstring4iTM3:@" " replacedByString:@"%20"]);
    if (status)
    ...
    [[NSWorkspace sharedWorkspace] openURL:[[[NSURL alloc] initWithString:
        [NSString stringWithFormat:@"help:openbook=%@",
            NSLocalizedStringFromTableInBundle(@"iTM2LaTeXHelpBook", @"HelpBooks",
                self.classBundle4iTM3, "Name")]] autorelease]];
    return;
}
