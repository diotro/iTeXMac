 ...
 *  Copyright (c) 2002 jlaurens@users.sourceforge.net. All rights reserved.
 */
...
{
    {
        ...
        NSLog(@"CountComponents: %u", CountComponents(&theComponentDesc));
        do
        {
                theLastComponent = FindNextComponent( theLastComponent, &theComponentDesc );
                NSLog(@"'%c%c%c%c', '%c%c%c%c'",
                    theComponentDesc.componentType / 256 / 256 / 256 % 256,
                    theComponentDesc.componentType / 256 / 256 % 256,
                    theComponentDesc.componentType / 256 % 256,
                    theComponentDesc.componentType % 256,
                        theComponentDesc.componentSubType / 256 / 256 / 256 % 256,
                        theComponentDesc.componentSubType / 256 / 256 % 256,
                        theComponentDesc.componentSubType / 256 % 256,
                        theComponentDesc.componentSubType % 256);
        }
        while( theLastComponent );
    }
    return;
}
#endif
...
{
        NDResourceFork * RF = [NDResourceFork resourceForkForReadingAtPath:aPath];
	NSData * theData = [RF dataForType:kOSAScriptResourceType Id:kScriptResourceID];
	...
	{
                NSLog(@"%@: No data in %@, %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd), RF, aPath);
		[self release];
		self = nil;
	}
...
- (OSAID)compileString:(NSString *)aString modeFlags:(long)aModeFlags
{
	...
	{				
		switch(OSACompile([self OSAComponent], &theScriptDesc, aModeFlags, &theCompiledScript))
                {
                    case errOSASystemError:
                        NSLog(@"errOSASystemError");
                        break;
                    case errOSAInvalidID:
                        NSLog(@"errOSAInvalidID");
                        break;
                    case errOSABadStorageType:
                        NSLog(@"errOSABadStorageType");
                        break;
                    case errOSAScriptError:
                        NSLog(@"errOSAScriptError");
                        break;
                    case errOSABadSelector:
                        NSLog(@"errOSABadSelector");
                        break;
                    case errOSASourceNotAvailable:
                         NSLog(@"errOSASourceNotAvailable");
                        break;
                    case errOSANoSuchDialect:
                        NSLog(@"errOSANoSuchDialect");
                        break;
                    case errOSADataFormatObsolete:
                        NSLog(@"errOSADataFormatObsolete");
                        break;
                    case errOSADataFormatTooNew:
                        NSLog(@"errOSADataFormatTooNew");
                        break;
                    case errOSACorruptData:
                        NSLog(@"errOSACorruptData");
                        break;
                    case errOSARecordingIsAlreadyOn:
                        NSLog(@"errOSARecordingIsAlreadyOn");
                        break;
                    case errOSAComponentMismatch:
                        NSLog(@"Parameters are from 2 different components");
                        break;
                    case errOSACantOpenComponent:
                        NSLog(@"Can't connect to scripting system with that ID");
                        break;
                }
		AEDisposeDesc( &theScriptDesc );
	}
...
//			break;
		case typeMagnitude:					//	unsigned 32-bit integer
		{
			...
			if( AEGetDescData(aDesc, &theInteger, sizeof(NSUInteger)) == noErr )
				theInstance = [NSNumber numberWithUnsignedInteger: theInteger];
			break;
		}
		case typeTrue:							//	TRUE Boolean value
