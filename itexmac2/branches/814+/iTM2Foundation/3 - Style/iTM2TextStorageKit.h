...
enum
{
	...
	kiTM2TextCommentSyntaxMask = kiTM2TextEndOfLineSyntaxMask << 1,//
	kiTM2TextPlaceholderSyntaxMask = kiTM2TextCommentSyntaxMask << 1// @ or _ are used as placeholders markers
};
...
enum 
{
	...
	kiTM2TextUnknownSyntaxMode = NSUIntegerMax & ~kiTM2TextFlagsSyntaxMask,
    kiTM2TextOuterDefaultSyntaxMode = (((1<<16)-1)|kiTM2TextEndOfLineSyntaxMask)
};
...
/*!
    ...
    @discussion Please, notice that the setter invalidates the glyphs of all the layout managers of the receiver.
...
*/
@property (assign) id syntaxParser;

/*!
    @property   syntaxParserStyle
...
*/
@property (readonly) NSString * syntaxParserStyle;

/*!
    @property   syntaxParserVariant
    @abstract   the syntax parser variant of the receiver
    @discussion Shortcut to the eponym method of the receiver's syntax parser.
    @param      None
    @result     an NSString identifier
*/
@property (readonly) NSString * syntaxParserVariant;
...
/*!
    ...
    @result     None
*/
@property (assign) id attributesChangeDelegate;


@property (assign) id stringController4iTM3;
@end
...
@private
    ...
    NSUInteger      iVarBadOff7Idx4iTM3;
    NSUInteger      iVarBadModeIdx4iTM3;
    NSUInteger      iVarPLI4iTM3;
    NSUInteger      iVarPL4iTM3;
@protected
    id __weak       iVarTS4iTM3;//    owner
    id __strong     iVarAS4iTM3;//   owned
}
...
/*!
    ...
    @discussion Description forthcoming.
    @param      None
    @result     An NSArray enumerator of unique strings.
*/
...
*/
+ (Class)syntaxParserClassForStyle:(NSString *)style;
...
@property (readonly,nonatomic) NSUInteger numberOfModeLines;
...
/*!
    ...
    @result     an index.
*/
@property (nonatomic,readonly) NSUInteger badOff7Index;

/*!
    @method     validateOff7sUpToIndex:
...
/*!
    @method     invalidateOff7sFromIndex:
...
/*!
    ...
    @discussion All the line modes with indices below this one have reliable mode information.
                Do not assume that this index is less than the number of lines.
    @param      None
    @result     an index.
*/
@property (nonatomic,readonly) NSUInteger badModeIndex;

/*!
    @method     validateModesUpToIndex:
...
/*!
    @method     invalidateModesFromIndex:
    @abstract   Modify eventually the first index for which the mode information may be bad.
    @discussion There is no automatic management of this index,
                subclassers must ensure that this index always points
                to the expected location with respect to its meaning.
...
/*!
    @method     lineIndexForLocation4iTM3:
    @abstract   The line index for the given location.
    @discussion The result is the first index for a mode line (ML) such that
                ML.startOff7 <= argument < ML.endOff7
                or the last mode line index if the previous inequalities cannot hold
                or NSUIntegerMax if there are no mode lines.
...
/*!
    @property   attributesServer
    @abstract   The attributes server of the receiver.
    @discussion Description forthcoming.
    @param      None
    @result     an iTM2TextAttributesServer instance.
*/
@property (assign,nonatomic) iTM2TextSyntaxParserAttributesServer * attributesServer;
...
/*!
    @method     textStorage
    @abstract   The text storage of the receiver. It owns the receiver.
    @discussion Description forthcoming.
    @param      None
    @result     an iTM2TextStorage instance.
*/
@property (assign) __weak id textStorage;
...
/*!
    @method     textStorageShouldReplaceCharactersInRange:withString:
...
*/
- (NSUInteger)validEOLModeOfModeLine:(iTM2ModeLine *)modeLine forPreviousMode:(NSUInteger)mode;
...
/*!
    @method     invalidateModesForCharacterRangeIndex:editedAttributesRangeIn:
...
@public
    // primitives
    NSUInteger _StartOff7;// global coordinates
    NSUInteger _UncommentedLength;//primitive, _UncommentedLength = NSUIntegerMax
    NSUInteger _CommentedLength;//primitive, _CommentedLength = NSUIntegerMax
    NSUInteger _EOLLength;//primitive
    // cached global coordinates
    NSUInteger _CommentOff7;//:=_StartOff7+_UncommentedLength, possibly NSUIntegerMax
    NSUInteger _ContentsEndOff7;//:= _StartOff7 + _UncommentedLength + _CommentedLength
    NSUInteger _EndOff7;//:= _StartOff7 + _Length
    // cached  local coordinates
    NSUInteger _Length;//_Length := _UncommentedLength + _CommentedLength + _EOLLength
    // Invalid range
    ...
    NSUInteger __strong * __SyntaxWordOff7s;
    NSUInteger __strong * __SyntaxWordLengths;
    NSUInteger __strong * __SyntaxWordEnds;
    NSUInteger __strong * __SyntaxWordModes;
    // flags
    NSUInteger _Status;
    NSUInteger _Depth;
...
/*!
    ...
    @result     None
*/
@property (readonly, nonatomic) NSUInteger EOLLength;

/*!
    @property   contentsLength
...
/*!
    @property   uncommentedLength
    @abstract   The length of the uncommented header of the line.
    @discussion Positive when there is a no starting comment character sequence.
				A comment is expected to start with a special character %, # are such examples
				The number of uncommented characters excluding the EOL.
    @param      argument
    @result     None
*/
...
/*!
    ...
    @result     None
*/
@property (readonly, nonatomic) NSUInteger commentedLength;
...
*/
@property (readonly, nonatomic) NSRange invalidLocalRange;

/*!
    @method     invalidGlobalRange
    @abstract   The invalid range of the receiver.
    @discussion The invalid range of the receiver is the range of all the location for which the mode information is not reliable.
                In fact there is a deterministic rule to compute the mode at location i knowing the mode at location i-1
                (or the previous mode if i is 0). For all the indices not in the invalid range, we have the following proposition:
    ...
    @result     a local coordinate range
*/
@property (readonly, nonatomic) NSRange invalidGlobalRange;
...
/*!
    @method     deleteModesInGlobalRange:
    ...
    @discussion The receiver will only manage the character range it is concerned with.
                ...
                The answer is YES if one of the modes inside the receiver has been removed at least partially.
                The answer is NO if the given ranges does not intersect with the receiver's one.
                If the answer is YES, the corresponding set of mode of the receiver is certainly not valid.
                If the answer is NO, the modes might be valid if changes before the receiver have not modified its prevous mode.
    @param      aRange
...
- (void)appendSyntaxModesAndLengths:(NSUInteger)firstMode,...;// convenient method for testing and debugging purposes only
...
/*!
    ...
    @discussion When positive, this value is the one used to compute all the valid modes declared in the receiver.
				...
				A line mode with an unknown or error syntax mode will not see its syntax modes fixed by the validEOL... method
				This means that the first line mode of a syntax parser MUST have a regular previous syntax mode.
    @param      None
    @result     mode
*/
...
/*!
    @property   startOff7
    @abstract   The startint offset of the receiver.
...
/*!
    @property   commentOff7
    @abstract   The comment offset of the receiver.
    @discussion Global coordinates.
    @param      None
    @result     An offset
*/
@property (readonly, nonatomic) NSUInteger commentOff7;
...
/*!
    ...
    @result     An offset
*/
@property (readonly, nonatomic) NSUInteger contentsEndOff7;

/*!
    @property   endOff7
    @abstract   The ending offset of the receiver.
    @discussion Global coordinates.
    @param      None
    @result     An offset
*/
@property (readonly, nonatomic) NSUInteger endOff7;

@property (readonly, nonatomic) NSRange completeRange;
@property (readonly, nonatomic) NSRange contentsRange;
@property (readonly, nonatomic) NSRange EOLRange;
...
/*!
    ...
    @abstract	The built in style URLs.
    @discussion	The built in style  URLs start from the framework containing the receiver,
...
/*!
    @method		otherStyleURLs
    @abstract	The other style  URLs.
    @discussion	They should be store in the various support folders at  URL
