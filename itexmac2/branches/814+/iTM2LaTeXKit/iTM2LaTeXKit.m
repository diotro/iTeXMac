...
{DIAGNOSTIC4iTM3;
//START4iTM3;
	sender.state = ([self contextBoolForKey:@"iTM2LaTeXToolbarShareConfiguration" domain:iTM2ContextAllDomainsMask]? NSOnState:NSOffState);
//END4iTM3;
	return YES;
}
	...
	else
	{
		...
		if([RE matchString:macro])
		{
			macro = [NSString stringWithFormat:
				([self contextBoolForKey:@"iTM2DontUseSmartMacros" domain:iTM2ContextAllDomainsMask]?@"%@{%@}%@":@"%@{__(%@)__}%@"),
